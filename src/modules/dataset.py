...
class hpa_hubmap_data_he(Dataset):
    def __init__(self, cfg, train=True, tfms=None, selection_tfms=None):
        ...
        kf = StratifiedKFold(
            n_splits=cfg["nfolds"], random_state=cfg["seed"], shuffle=True
        )
        ids = set(
            ids[list(kf.split(ids, organs))[cfg["fold"]][0 if train else 1]]
        )

        self.fnames = [
            fname
            for fname in os.listdir(self.TRAIN_IMGS)
            if fname.split(".")[0] in ids
        ]
        self.train = train
        self.tfms = tfms
        ...
        self.T = torchtf.Compose(
            [torchtf.ToTensor(), torchtf.Lambda(lambda x: x * 255)]
        )
    ...
    def __getitem__(self, idx):
        ...
        img = cv2.cvtColor(
            cv2.imread(os.path.join(self.TRAIN_IMGS, fname)), cv2.COLOR_BGR2RGB
        )
        mask = cv2.imread(
            os.path.join(self.MASKS, fname), cv2.IMREAD_GRAYSCALE
        )
...
class hpa_hubmap_data_val(Dataset):
    def __init__(self, ids=None, fold=None, train=True, tfms=None):
        self.fnames = [
            fname for fname in os.listdir(TRAIN) if fname.split(".")[0] in ids
        ]
        self.train = train
        self.tfms = tfms
    ...
    def __getitem__(self, idx):
        fname = self.fnames[idx]
        img = cv2.cvtColor(
            cv2.imread(os.path.join(TRAIN, fname)), cv2.COLOR_BGR2RGB
        )
...
class neptune_data(Dataset):
    ...
    def __getitem__(self, idx):
        fname = self.fnames[idx]
        img = cv2.cvtColor(
            cv2.imread(os.path.join(self.root, fname)), cv2.COLOR_BGR2RGB
        )

        mask_name = fname.split(".")[0]
        mask_name += (
            "_mask_capsule.png" if "HE_glom" not in self.root else "_mask.png"
        )

        mask = cv2.imread(
            os.path.join(self.root, mask_name), cv2.IMREAD_GRAYSCALE
        )
...
class aidpath_data(Dataset):
    ...
    def __getitem__(self, idx):
        fname = self.fnames[idx]
        img = cv2.cvtColor(
            cv2.imread(os.path.join(self.path_imgs, fname)), cv2.COLOR_BGR2RGB
        )
        mask = cv2.imread(
            os.path.join(self.path_masks, fname), cv2.IMREAD_GRAYSCALE
        )
