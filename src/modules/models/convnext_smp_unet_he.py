...
from .isw import ISW
from .stinv_training import domain_predictor, GradientReversal
...
class ConvNextUNet(nn.Module):
    ...
    def load_pretrain(self):
        ...
        state_dict = torch.load(pretrain, map_location=lambda storage, loc: storage)['model']  # True
    ...
    def __init__(self,
        ...
        self.aux = nn.ModuleList([
            nn.Conv2d(encoder_dim[i], n_classes, kernel_size=1, padding=0) for i in range(len(encoder_dim))
        ])
            ...
            if self.filter_sensitive:
                ...
                with torch.no_grad():
                    with torch.cuda.amp.autocast(enabled = True):
                        fmaps_transformed = self.encoder.downsample_layers[0](batch['image_tf'])
