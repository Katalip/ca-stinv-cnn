...
T = torchtf.Compose([torchtf.ToTensor(), torchtf.Lambda(lambda x: x * 255)])
...
class stain_aug:
    ...
    def augment(self, image):
        ...
        if self.normalizer:
            ...
            try:
                stain_vectors_src, _, maxC_src = normalizer.get_stain_matrix(
                    src
                )
            except Exception as e:
                print(f"Lapack error -> returning img itself")
                return image

            if self.type == "light":
                alpha_sig = np.random.uniform(1 - self.sigma1, 1 + self.sigma1)
                beta_sig = np.random.uniform(-self.sigma2, self.sigma2)
                img, _, _ = normalizer.normalize(
                    src,
                    maxC_src,
                    stain_vectors_src * alpha_sig + beta_sig,
                    stains=False,
                )

            elif self.type == "strong":
                for index in [0, 1, 2]:
                    alpha_sig = np.random.uniform(
                        1 - self.sigma1, 1 + self.sigma1
                    )
                    beta_sig = np.random.uniform(-self.sigma2, self.sigma2)
                    stain_vectors_src[index] = (
                        stain_vectors_src[index] * alpha_sig + beta_sig
                    )

                img, _, _ = normalizer.normalize(
                    src, maxC_src, stain_vectors_src, stains=False
                )
