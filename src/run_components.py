...
from modules.dataset import (
    aidpath_data,
    hpa_hubmap_data_he,
    hubmap21_kidney_data,
    neptune_data,
)
from modules.metrics import DicePrecisionRecall, mean_Dice
...
def train_model(model, train_loader, val_loader, optimizer, scaler, cfg):
    ...
    wandb.init(
        ...
        name=cfg["Train"].get("experiment_name"),
        resume=True,
    )
    ...
    for epoch in range(
        cfg["Train"].get("start_epoch"), cfg["Train"].get("epochs")
    ):
        ...
        for data in train_loader:
            ...
            p = float(iter + epoch * iterations_per_epoch) / (
                cfg["Train"].get("epochs") * iterations_per_epoch
            )
        ...
        elapsed = time.strftime(
            "%Hh:%Mm:%Ss", time.gmtime(end_time - start_time)
        )
        ...
        if epoch % cfg["Train"].get("val_epoch") == 0 or (
            epoch % cfg["Train"].get("checkpoint_epoch") == 0 and epoch > 0
        ):
            save_preds_flag = epoch % cfg["Train"].get("checkpoint_epoch") == 0
            val_loss, val_score = evaluate_model(
                model, val_loader, metric, cfg, save_preds=save_preds_flag
            )
        ...
        if (
            epoch % cfg["Train"].get("checkpoint_epoch") == 0
            or epoch == cfg["Train"].get("epochs") - 1
            or epoch > cfg["Train"].get("epochs") - 20
        ) and (epoch > 0):
            save_model(
                model, f"{cfg['Train'].get('save_path')}/epoch_{epoch}.pth"
            )
        ...
        for i, data in enumerate(val_loader):
            ...
            if save_preds:
                save_predictions_as_imgs(
                    outputs["raw"],
                    mask,
                    folder=f"{cfg['Train'].get('save_path')}",
                    idx=i,
                )
...
def test_external(
    model, val_loader, metric, device="GPU", save_path=None, save_preds=False
):
    model.eval()
    model.output_type = ["loss"]
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(val_loader):
            ...
            if save_preds:
                save_predictions_as_imgs(
                    outputs["raw"], mask, folder=f"{save_path}", idx=i
                )
...
def eval_hpa_hubmap22(
    cfg, model, state_dict, log, device, save_path, batch_size=4
):
    ...
    loss, dice, precision, recall = test_external(
        model, val_loader, metric, device, save_path
    )
...
def eval_neptune(
    cfg, model, state_dict, log, device, save_path, batch_size=4, img_size=480
):
    metric = DicePrecisionRecall()
...
def eval_aidpath(
    cfg, model, state_dict, log, device, save_path, img_size=256, batch_size=4
):
    metric = DicePrecisionRecall()
...
def eval_hubmap_kidney(
    cfg, model, state_dict, log, device, save_path, img_size=224, batch_size=4
):
    metric = DicePrecisionRecall()
