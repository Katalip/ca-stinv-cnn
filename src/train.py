...
def main(cfg):
    ...
    train_loader = DataLoader(train_data, shuffle=True, batch_size=cfg['Loader'].get('batch_size'), num_workers=cfg['Loader'].get('num_workers'))
    val_loader = DataLoader(val_data, shuffle=True, batch_size=cfg['Loader'].get('batch_size'), num_workers=cfg['Loader'].get('num_workers'))
    ...
    if cfg['Architecture'].get('encoder') == 'resnet50':
        ...
        model = ConvNextUNet(stinv_training=True, stinv_enc_dim=0, pool_out_size=6, filter_sensitive=True, n_domains=6, domain_pool='max', encoder_pretrain=cfg['Architecture'].get('weights')).cuda()
        model.load_pretrain()

    if cfg['Train'].get('start_epoch') > 0:
        model.load_state_dict(torch.load(f"{cfg['Train'].get('save_path')}/epoch_{cfg['Train'].get('start_epoch')}.pth"))
